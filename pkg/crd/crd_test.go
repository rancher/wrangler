...
func TestBatchCreateCRDs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(mock *MockCustomResourceDefinitionInterface) {
				...
				mock.EXPECT().List(gomock.Any(), gomock.Any()).Return(list, nil)
				...
				mock.EXPECT().List(gomock.Any(), gomock.Any()).Return(list2, nil)
			},
		},
		{
			...
			setupMock: func(mock *MockCustomResourceDefinitionInterface) {
				...
				mock.EXPECT().List(gomock.Any(), gomock.Any()).Return(nil, testErr)
		...
		},
		{
			...
			setupMock: func(mock *MockCustomResourceDefinitionInterface) {
				...
				mock.EXPECT().Create(gomock.Any(), crd1, gomock.Any()).Return(nil, testErr)
			},
		},
		{
			name:         "update CRD",
		...
		},
		{
			...
			setupMock: func(mock *MockCustomResourceDefinitionInterface) {
				...
				mock.EXPECT().Update(gomock.Any(), crd1, gomock.Any()).Return(nil, testErr)
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := BatchCreateCRDs(context.Background(), mock, tt.toCreateCRDs); (err != nil) != tt.wantErr {
