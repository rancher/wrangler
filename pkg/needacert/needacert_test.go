...
func TestHandler_scheduleNextCertCheck_TableDriven(t *testing.T) {
	now := time.Now()

	// Minimal mock for services.EnqueueAfter
	...
	tests := []struct {
	...
	}{
		{
			...
			cert:      &x509.Certificate{NotAfter: now.Add(30 * 24 * time.Hour)},
			wantDelay: 29 * 24 * time.Hour,
			wantErr:   false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			obj := &corev1.Service{}
			err := h.scheduleNextCertCheck(obj, tt.cert)
